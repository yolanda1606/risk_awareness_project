...
void TsdfServer::getServerConfigFromRosParam() {
  ...
  node_->declare_parameter("generate_esdf", false);
  node_->declare_parameter("publish_esdf_map", false);

  // Use local temporary variables to read them (since class members don't exist)
  bool generate_esdf_temp = false;
  bool publish_esdf_map_temp = false;
  
  node_->get_parameter("generate_esdf", generate_esdf_temp);
  node_->get_parameter("publish_esdf_map", publish_esdf_map_temp);
